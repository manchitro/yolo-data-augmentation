...
def draw_yolo(image, labels, file_name, seed):
    """
    ...
    cv2.imwrite(f"bb_image/{file_name}_{str(seed)}.png", image)
...
def get_augmented_results(image, bboxes, seed):
    """
...
def save_augmentation(trans_image, trans_bboxes, trans_file_name, seed):
    """
    ...
    if tot_objs:
        ...
        if not has_negative_element(trans_bboxes):
            ...
            save_aug_lab(trans_bboxes, CONSTANTS["out_lab_pth"]+"_"+str(seed), trans_file_name + "_" + str(seed) + ".txt")
            save_aug_image(trans_image, CONSTANTS["out_img_pth"]+"_"+str(seed), trans_file_name + "_" + str(seed) + ".jpg")
            # Draw bounding boxes on the augmented image
            draw_yolo(trans_image, trans_bboxes, trans_file_name, seed)
        else:
